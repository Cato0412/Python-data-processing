...
for i in range(1,6):
    ...
    #totales[f"S{i}"]=COLVARIABLESS.sum(axis=1)
    ...
    dfRUTOT[f"S{i}-VISITAS"]=dfRUTOT[f"S{i}-VISITAS"].fillna(0)
    dfRUTOT[f"S{i}-VISITAS"]=dfRUTOT[f"S{i}-VISITAS"].replace("",0)
...
#Carpeta donde se reciben los nuevos archivos
    NOMBREFINAL=f"SEMANAS OCT\\S{i}.xlsx"
